...
通过 Hermes Agent 的 MCP Chrome 工具完成。详见 skills/maimai-search/
"""
...
class MaimaiSearcher(BaseSearcher):
    ...
    def save_cookies(self, cookies: List[Dict]) -> None:
        ...
        logger.info(f"[脉脉] Cookie 已保存: {len(cookies)} 条")

    def check_cookie(self) -> bool:
        """检查 Cookie 是否有效"""
        cookies = self.cookie_manager.load()
        if not cookies:
            logger.warning(f"[脉脉] 账号 '{self.account}' 未找到 Cookie")
            return False

        has_valid_cookie = any(
            c.get("name") in ["access_token", "uid"] for c in cookies
        )

        if has_valid_cookie:
            logger.info(f"[脉脉] 账号 '{self.account}' Cookie 有效")
            self._cookies = cookies
            self._is_logged_in = True
            return True
        else:
            logger.warning(f"[脉脉] 账号 '{self.account}' Cookie 已过期")
            return False
    ...
    def parse_candidate_list(self, page_data: Dict[str, Any]) -> List[Dict]:
        ...
        for elem in elements:
            if "search-result-item" in elem.get("class", ""):
    ...
    async def search(
        ...
        logger.info(f"[脉脉] 开始搜索: '{keyword}'")
        logger.info(f"  城市: {city or '不限'}, 页数: {pages}")
        logger.info("  请通过 Hermes Agent 执行搜索流程，详见 skills/maimai-search/")
        return []
